...
import {
  LayoutDashboard, Bot, CreditCard, Shuffle, Settings2, Plug,
  Activity, User,
} from 'lucide-react';
...
export default function Sidebar({ locale, dict }: Props) {
  ...
  return (
    <aside className="w-60 shrink-0 border-r border-white/5 bg-ink-900/60 min-h-screen p-4 hidden md:flex md:flex-col">
      <Link href={`/${locale}`} className="flex items-center gap-2 mb-6">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-brand-500 to-fuchsia-500 grid place-items-center font-bold text-white">D</div>
        <span className="text-lg font-bold text-white">DC AgentPay X</span>
      </Link>
      ...
      {isAuthenticated ? (
        ...
        // Not authenticated → keep sidebar minimal.
        // The Topbar and each guarded page already show a Login button/prompt.
